...
# 잔고 저장
jango_yaml_path = './config/jango.yaml'
...
class Cp6033:
    ...
    def rq6033(self, retcode):
        ...
        for i in range(cnt):
            ...
            # Yaml 파일에 데이터 저장
            jango_yaml_file.write(code + ":" + "\n")            
            jango_yaml_file.write("  " + "- " + str(amount) + "\n")
            jango_yaml_file.write("  " + "- " + str(int(buyPrice)) + "\n")
            jango_yaml_file.write("  " + "- " + name + "\n")
        # 사용한 Yaml 파일 닫기
        jango_yaml_file.close()
